    ...
    class Program
    {
        ...
        public static void SubscribeReactiveAnyType()
        {
            ...
            using (TcAdsClient client = new TcAdsClient())
            {
                ...
                dict.Add("TwinCAT_SystemInfoVarList._TaskInfo[1].CycleCount", typeof(ushort));
                dict.Add("TwinCAT_SystemInfoVarList._TaskInfo[1].LastExecTime", typeof(uint));

                // Reactive Notification Handler
                var valueObserver = Observer.Create<NotificationEx>(val =>
                {
                    Console.WriteLine(string.Format("Handle: {0}, Value: {1}", val.NotificationHandle, val.ToString()));
                }
                );

                // Turning ADS Notifications into sequences of Value Objects (Taking 20 Values)
                // and subscribe to them.
                IDisposable subscription = client.WhenNotification(dict,NotificationSettings.Default,null).Take(20).Subscribe(valueObserver);
...
#region CODE_SAMPLE_SYMBOLS
            ...
            using (TcAdsClient client = new TcAdsClient())
            {
                ...
                // Turning ADS Notifications into sequences of Value Objects (Taking 20 Values)
                // and subscribe to them.
                IDisposable subscription = symbols.WhenValueChanged().Take(20).Subscribe(valueObserver);
        ...
        public static void SubscribeReactiveValueChanges()
        {
            ...
            using (TcAdsClient client = new TcAdsClient())
            {
                ...
                // Reactive Notification Handler
                var valueObserver = Observer.Create<ValueChangedArgs>(args =>
                {
                    Console.WriteLine(string.Format("{0} {1:u} {2} = '{3}' ({4})", eventCount++, args.UtcLocalSystemTime, args.Symbol.InstancePath, args.Value, args.Symbol.DataType.Name));
                }
                );
                ...
                IDisposable subscription = allBoolean.WhenValueChangedAnnotated().Subscribe(valueObserver);
