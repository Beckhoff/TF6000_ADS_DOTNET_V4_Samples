...
#pragma warning restore 0618
		...
		{						
			...
			try
			{
				if( !cbFlat.Checked ) 
				{
                    Stack<TcAdsSymbolInfo> parents = new Stack<TcAdsSymbolInfo>();

					TcAdsSymbolInfo symbol = symbolLoader.GetFirstSymbol(true);

                    foreach(TcAdsSymbolInfo subSymbol in symbol.SubSymbols)
                    {
		...
		private void btnReadSymbolInfo_Click(object sender, System.EventArgs e)
		{
			try
			{
				ITcAdsSymbol5 symbol = (ITcAdsSymbol5)adsClient.ReadSymbolInfo(tbSymbolname.Text);
				if( symbol == null)
				{
		...
		{	
			try
			{			
				ITcAdsSymbol5 symbol = (ITcAdsSymbol5)symbolLoader.FindSymbol(tbSymbolname.Text);
				if( symbol == null)
				{
					MessageBox.Show("Symbol " + tbSymbolname.Text + " not found");
					return;
		...
		private void treeViewSymbols_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			if( e.Node.Text.Length > 0 )
			{
				...
				{
					SetSymbolInfo((ITcAdsSymbol5)e.Node.Tag);
				}
			}
		}
		
		private TreeNode CreateNewNode(TcAdsSymbolInfo symbol, Stack<TcAdsSymbolInfo> parents)
		{
			TreeNode node = new TreeNode(symbol.Name);			
			node.Tag = symbol;
            
		    if (!symbol.IsRecursive(parents))
		    {
		        parents.Push(symbol);
		...
		private void SetSymbolInfo(ITcAdsSymbol5 symbol)
		{
